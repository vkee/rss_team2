...
public class GripperController {
	...
	protected double LINE_THETA_INTERCEPT; // intercept of the line between
											// PWM_90 and PWM_0
	protected int MAX_PWM_CHANGE; // the largest PWM change in value that can be
									// safety written to the servo (for 1 radian
									// of rotation)
...
//		super(minPWM, maxPWM, thetaRange, pwm0, pwm270);
	...
	/**
	 * Computes the PWM to write to the servo taking into the constraint of not
	 * moving more than 1 radian per control step
	 * 
	 * @param currPWM
	 *            the last PWM value written to the servo
	 * @param up
	 *            whether the servo is moving up
	 * @return the PWM value to be written to the servo
	 */
	public int fullRotation(int currPWM, boolean up) {
		// int correction;
		if (up) {
			return Math.max(Math.min(currPWM + (MAX_PWM - MIN_PWM)
					/ SHIFT_AMOUNT, MAX_PWM), MIN_PWM);
			// correction = Math.min(MAX_PWM - currPWM, MAX_PWM_CHANGE);
			// return currPWM + correction;
		} else {
			return Math.min(Math.max(currPWM - (MAX_PWM - MIN_PWM)
					/ SHIFT_AMOUNT, MIN_PWM), MAX_PWM);
