...
public class Image {
     ...
     * @return The amount of red in the pixel at the specified location.
     */
    public int getPixelRedInt(int x, int y) {
	return Image.pixelByteToInt(getPixelChannel(x, y, Channel.RED));
    }
     ...
     * @return The amount of green in the pixel at the specified location.
     */
    public int getPixelGreenInt(int x, int y) {
	return Image.pixelByteToInt(getPixelChannel(x, y, Channel.GREEN));
    }
     ...
     * @return The amount of blue in the pixel at the specified location.
     */
    public int getPixelBlueInt(int x, int y) {
	return Image.pixelByteToInt(getPixelChannel(x, y, Channel.BLUE));
    }
    
    /**
     * Returns a float of length 3 (hue, saturation, brightness) of the pixel
     * at location (x,y)
     * 
     * @param x
     *            The x coordintate of the pixel.
     * @param y
     *            The y coordinate of the pixel.
     * @return A 3-tuple of (hue, saturation, brightness)
     */
    public float[] getPixelHSB(int x, int y) {
	int red = getPixelRedInt(x, y);
	int green = getPixelGreenInt(x, y);
	int blue = getPixelBlueInt(x, y);
    ...
    /**
     * Convert an RGB signed byte to an integer between 0 and 255
     * 
     * @param val
     *            byte value
     * @return val in integer format
     */
    public static int pixelByteToInt(byte val) {
	int result = (new Byte(val)).intValue();
