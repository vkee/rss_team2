...
public class BlobTracking {
     ...
     *
     * @param src the src RGB image, not packed
	...
	for (int i = 0; i < src.getWidth(); i++) {
		...
		if(red>blue*1.4 && red>green*1.4){
		    ...
		    dest.setPixel(i,j,(byte)value,(byte)value,(byte)value);
		}
		...
		dest.setPixel(centroid_i,centroid_j,(byte)0,(byte)255,(byte)0);
		for (int k = 0; k<4; k++){
		    dest.setPixel(centroid_i+k,centroid_j+k,(byte)0,(byte)255,(byte)0);
		    dest.setPixel(centroid_i-k,centroid_j-k,(byte)0,(byte)255,(byte)0);		
		    dest.setPixel(centroid_i+k,centroid_j-k,(byte)0,(byte)255,(byte)0);
		    dest.setPixel(centroid_i-k,centroid_j+k,(byte)0,(byte)255,(byte)0);
    ...
    /*blobPresent(src,dest);
		...
		int red = src.getPixelRed(i,j);
				...
				if(src.isHue(src.getPixel(i,j),hues[preference],0.05,hsb)){
					dest.setPixel(i, j, 255,255,255); //replaces desired pixels with white pixels.
					numberOfPixels++;
					redCoordinateSumX += i;
					redCoordinateSumY += j;
...
/		}
		...
		if(numberOfPixels !=0){
			redCoordinateSumX /= (long)numberOfPixels;
			redCoordinateSumY /= (long)numberOfPixels;
		}
		if(redCoordinateSumX > 0 && redCoordinateSumY > 0){
			System.err.println("Ball Found : Estimated center at X -> " + redCoordinateSumX + "Y -> " +redCoordinateSumY);
		}
		//End Student Code
	...
	public void classify(Image src,Image dest){
		...
		for (int i=0;i<src.getWidth();i++){
			for(int j=0;j<src.getHeight();j++){
				...
				if(src.isHue(src.getPixel(i,j),hues[preference],0.05,hsb)){
					if (preference == RED){
						dest.setPixel(i,j,255,0,0);
					}else if (preference==GREEN){
						dest.setPixel(i,j,0,255,0);
					} else{
...
*/
	...
	double ballCameraDistance= Math.sqrt(ballImageAreaAtOneMeter/ballImageArea);
	...
	//Assumes that the camera is perfectly in the center of the robot and that it is image is linear; which is a lie, but it is a good approximation
	//Assumes that there is a linear conversion between ballImageX and ballCameraX
	//double ballCameraX=ballImageX*imageToCameraXConversion;
	
	//Converts now from the camera frame to the robot frame. Since the camera is in the center of the robotm this conversion will be simple.
	double ballRobotX=0;//ballCameraX;
	double ballRobotY;
	if(ballRobotX<ballRobotDistance){
	    ballRobotY=Math.sqrt(ballRobotDistance*ballRobotDistance - ballRobotX*ballRobotX);
	}
	else{ //Should never happen, but if happens, we will define the distance in y to be the total distance, to protect against imaginary numbers
	...
	public int[] blobPresent(Image src, Image dest){
		...
		for(int i=0;i<dest.getWidth();i++){
			for(int j=0;j<dest.getHeight();j++){
				int nextPixel= dest.getPixel(i,j);
				byte nextPixelRed=dest.getPixelRed(i,j);
				byte nextPixelGreen=dest.getPixelGreen(i,j);
				byte nextPixelBlue=dest.getPixelBlue(i,j);

				boolean condition = (preference == RED   && nextPixelRed == 255 && nextPixelGreen==   0 && nextPixelBlue==   0) ||
				                    (preference == GREEN && nextPixelRed ==   0 && nextPixelGreen == 255 && nextPixelBlue ==   0) ||
				                    (preference == BLUE  && nextPixelRed ==   0 && nextPixelGreen ==   0 && nextPixelBlue == 255);
				if (condition){
					matchingPixels++;
	...
	if(matchingPixelsXCount > 0 && matchingPixelsYCount > 0){
		for (int i = (int)matchingPixelsXCount - 8; i< (int)matchingPixelsXCount + 9;i++){
			if(i < dest.getWidth() && i > 0){
				dest.setPixel(i,(int)matchingPixelsYCount,255,255,255);
        	}
    	}
    	for (int i = (int)matchingPixelsYCount - 8; i< (int)matchingPixelsYCount + 9;i++){
			if(i < dest.getHeight() && i > 0){
				dest.setPixel((int)matchingPixelsXCount ,i,255,255,255);
			}
		}
	}
	if (DRAW_HISTOGRAM) {
		...
		centroidX=returnArray[1];
		centroidY=returnArray[2];
...
/*  
  	for (int i = 0; i < w; i++) {
      		for (int j = 0; j <h; j++) {
        		int redPixel = src.getPixelRed(i,j);
			if (redPixel < 0){
				redPixel += 256;
		}
			int greenPixel = src.getPixelGreen(i,j);
			if (greenPixel < 0){
				greenPixel += 256;
		}
			int bluePixel = src.getPixelBlue(i,j);
			if (bluePixel < 0){
				bluePixel += 256;
		}
