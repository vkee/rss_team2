 ...
 *
 */
public class LocalizationTestComplex implements NodeMain {
	...
	public LocalizationTestComplex() {
		cSpace = new CSpace();
	}
	...
	public void onStart(Node node) {
		...
		try {
			...
			try {
				...
				particleFilter = new ParticleFilter(robotStartPos.x, robotStartPos.y, 0.0, PARTICLE_FILTER_RADIUS, 10000, challengeMap, 0.025, 0.025, 5.0);
				...
				for (Point2D.Double pt : rrtPath){
					...
					Thread.sleep(2000); // Waiting 5 seconds between each step
				}
				//                particleFilter.printParticles();
	...
	private ArrayList<Point2D.Double> generateTestPath(Double startPoint){
		...
		testPath.add(new Point2D.Double(0.8, 0.0));
