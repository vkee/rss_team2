...
public class NeckScan implements FSMState {
	...
	/**
	 * Determines the distances from the robot to the fiducials in the robot's
	 * FOV
	 * 
	 * @param robotPos
	 *            the robot's current position
	 * @param measuredFiducials
	 *            the indices of the fiducials in the robot's FOV
	 * @return the distances from the robot's position to the fiducials
	 */
	// TODO: SHOULDNT THIS BE IN PARTICLE FILTER CODE NOT HERE??
	private HashMap<Integer, java.lang.Double> getFidsDists(
			Point2D.Double robotPos, ArrayList<Integer> measuredFiducials) {
		...
		for (Integer index : measuredFiducials) {
			...
			// Potential bug site is if robot position at 0,0 and map goes
			// negative,
			// but this should be able to account for it in this ordering
			double dist = RRT.getDist(robotPos.x, robotPos.y, fidPos.x,
					fidPos.y);
			// System.out.println("Distance to Fiducial " + index + " at " +
			// fidPos + " is " + dist);
