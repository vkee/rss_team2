    ...
    public void update(GenericMessage msg) {
        ...
        for (FiducialObject fids : detectedFids) {
            ...
            if (totalFidDist.containsKey(fidIndex)){
                totalFidDist.put(fidIndex, totalFidDist.get(fidIndex) + fidDist);
            } else {
            ...
            if (numMeasurements.containsKey(fidIndex)){
                numMeasurements.put(fidIndex, numMeasurements.get(fidIndex) + 1);
            } else {
        ...
        for (Integer index : totalFidDist.keySet()) {
            double avgDist = totalFidDist.get(index)/numMeasurements.get(index);
